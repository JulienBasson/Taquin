...
public class IterativeDeepeningAStar implements Algorithm {
    ...
    public List<Direction> solve(State from){
        ...
        while(true){
            ...
            if(t.found) return t.path;
            if(t.cost == Double.POSITIVE_INFINITY) return null;
            bound = t.cost;
        }
    }

    private Path search(State node, double costAlready, double bound, List<Direction> previousMoves){
